...
type Props = {
  ...
  remove?: (id: string) => void; // removeは省略可能
};
      ...
      {todos.map((todo) => (
        ...
        >
          ...
          <div className="flex items-baseline justify-between text-slate-700">
            ...
            {props.remove && ( // removeが渡されている場合のみ削除ボタンを表示
              <button
                onClick={() => {
                  props.remove && props.remove(todo.id);
                }}
                className="ml-4 rounded-md bg-slate-200 px-2 py-1 text-sm font-bold text-white hover:bg-red-500"
              >
                削除
              </button>
            )}
